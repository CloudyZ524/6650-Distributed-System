...
public class PostingThread implements Runnable {
  ...
  private List<Long> latenciesList;
  ...
  public PostingThread(BlockingQueue<LiftRide> queue, int numRequests,
      ...
      List<Long> latenciesList, PrintWriter fileWriter) {
    this.queue = queue;
    this.numRequests = numRequests;
    this.successfulRequests = successfulRequests;
    this.failedRequests = failedRequests;
    this.count = count;
    this.latenciesList = latenciesList;
    ...
    for (int i = 0; i < numRequests; i++) {
      ...
      while (!success && retries < MAX_RETRIES) {
        try {
          ...
          synchronized (latenciesList) {
            latenciesList.add(latency);
          }
          synchronized (fileWriter) {
            fileWriter.println(startTime + ",POST," + latency + "," + responseCode);
          }
        }
      }
      if (!success) {
  ...
  private SkiersApi createApiClient() {
    ...
    apiClient.setBasePath("http://54.185.40.136:8080/servlet_war");
