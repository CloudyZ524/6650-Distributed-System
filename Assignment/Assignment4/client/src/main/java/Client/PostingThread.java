...
public class PostingThread implements Runnable {
  ...
  final private int RESORT_ID = 1;
  final private int SKIER_ID_RANGE = 100000;
  final private int DAY_ID_RANGE = 3;
  ...
  public void run() {
    ...
    for (int i = 0; i < numRequests; i++) {
      ...
      try {
        ApiResponse<Void> apiResponse = skiersApi.writeNewLiftRideWithHttpInfo(liftRide, RESORT_ID, SEASON_ID, String.valueOf(random.nextInt(DAY_ID_RANGE) + 1), random.nextInt(SKIER_ID_RANGE) + 1);
